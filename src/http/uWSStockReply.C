/*
 * Copyright (C) 2021 Guinebert Sylvain, Paris, France.
 *
 * All rights reserved.
 */
//
...
namespace http
{
  namespace server
  {
    ...
    namespace stock_replies
    {
      ...
      const std::string ok = ""; //std::string_view ?
      ...
      std::string_view toText(Reply::status_type status)
      {
        switch (status)
        {
        case Reply::ok:
      ...
      std::string_view toName(Reply::status_type status)
      {
        switch (status)
        {
        case Reply::ok:
      ...
      void buildOriginalURL(uWS::HttpRequest *req, std::string &url)
      {
        if (url.empty())
        {
          url = "http://";
    ...
    uWSStockReply::uWSStockReply(const Configuration &configuration)
        : configuration_(configuration),
          transmitted_(false)
    {
    }

    uWSStockReply::uWSStockReply(std::string extraContent,
    ...
    std::string uWSStockReply::contentType()
    {
      return "text/html";
    }

    // ::int64_t uWSStockReply::contentLength()
    // {
    //   std::string full_path(configuration_.errRoot() + stock_replies::toName(status()));
    ...
    //                      escapedUrl);
    //         index += clen;
    //       } while ((index = line.find("<-- ORIGINAL URL ESCAPED -->", index) != line.npos));
    //     }
    //     content += line + "\r\n";
    //   }
    //   ifstr.close();
    ...
    void uWSStockReply::send(uWS::HttpResponse<false> *res, Reply::status_type status)
    {
        res->writeHeader("Content-Type", "text/html")
            ->end(stock_replies::toText(status));
    }
    void uWSStockReply::send(uWS::HttpResponse<true> *res, Reply::status_type status)
    {
        res->writeHeader("Content-Type", "text/html")
            ->end(stock_replies::toText(status));
    }

    std::string_view uWSStockReply::get(Reply::status_type status)
    {
      return stock_replies::toText(status);
    }
