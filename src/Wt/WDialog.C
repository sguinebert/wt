...
private:
    ...
    void coverFor(WDialog *dialog, const WAnimation& animation) {
        if (dialog) {
            ...
            auto js = fmt::format("setTimeout(function() {{{}._p_.updateGlobal('{}') }});",
                                  WApplication::instance()->javaScriptClass(), dialog->layoutContainer_->id());

            dialog->doJSAfterLoad(js);
            ...
            WApplication::instance()->doJavaScript("setTimeout(function() {{{}._p_.updateGlobal(null) }});",
...
void WDialog::render(WFlags<RenderFlag> flags)
{
     ...
     */
        ...
        auto js = fmt::format(FMT_COMPILE("new " WT_CLASS ".WDialog({},{},{},{:d},{:d},{:d},\"{}\",\"{}\",\"{}\");"),
                              ...
                              centerX,
                              centerY,
                              (moved_.isConnected() ? moved_.name() : "null"),
                              (resized_.isConnected() ? resized_.name() : "null"),
                              zIndexChanged_.name());
        ...
        if (!app->environment().agentIsIElt(9) &&
            !app->environment().ajax()) {
#warning "TODO: check if this is still needed"
