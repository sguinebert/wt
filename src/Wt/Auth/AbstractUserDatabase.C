...
{
  LOG_ERROR(fmt::runtime(Require("setStatus()").what()));
}

PasswordHash AbstractUserDatabase::password(const User& user) const
{
  LOG_ERROR(fmt::runtime(Require("password()", PASSWORDS).what()));
...
{
  LOG_ERROR(fmt::runtime(Require("setPassword()", PASSWORDS).what()));
}
...
{
  LOG_ERROR(fmt::runtime(Require("registerNew()", REGISTRATION).what()));
...
{
  LOG_ERROR(fmt::runtime(Require("deleteUser()", REGISTRATION).what()));
}

std::string AbstractUserDatabase::email(const User& user) const
{
  LOG_ERROR(fmt::runtime(Require("email()", EMAIL_VERIFICATION).what()));
...
{
  LOG_ERROR(fmt::runtime(Require("setEmail()", EMAIL_VERIFICATION).what()));
  return false;
}

User AbstractUserDatabase::findWithEmail(const std::string& address) const
{
  LOG_ERROR(fmt::runtime(Require("findWithEmail()", EMAIL_VERIFICATION).what()));
...
{
  LOG_ERROR(fmt::runtime(Require("unverifiedEmail()", EMAIL_VERIFICATION).what()));
...
void AbstractUserDatabase::setUnverifiedEmail(const User& user,
					      const std::string& address)
{
  LOG_ERROR(fmt::runtime(Require("setUnverifiedEmail()", EMAIL_VERIFICATION).what()));
}

Token AbstractUserDatabase::emailToken(const User& user) const
{
  LOG_ERROR(fmt::runtime(Require("emailToken()", EMAIL_VERIFICATION).what()));
...
EmailTokenRole AbstractUserDatabase::emailTokenRole(const User& user)
  const
{
  LOG_ERROR(fmt::runtime(Require("emailTokenRole()", EMAIL_VERIFICATION).what()));
...
void AbstractUserDatabase::setEmailToken(const User& user, const Token& token,
					 EmailTokenRole role)
{
  LOG_ERROR(fmt::runtime(Require("setEmailToken()", EMAIL_VERIFICATION).what()));
}

User AbstractUserDatabase::findWithEmailToken(const std::string& hash) const
{
  LOG_ERROR(fmt::runtime(Require("findWithEmailToken()", EMAIL_VERIFICATION).what()));
...
{
  LOG_ERROR(fmt::runtime(Require("addAuthToken()", AUTH_TOKEN).what()));
}

void AbstractUserDatabase::removeAuthToken(const User& user,
					   const std::string& hash)
{
  LOG_ERROR(fmt::runtime(Require("removeAuthToken()", AUTH_TOKEN).what()));
}
...
{
  LOG_WARN(fmt::runtime(Require("updateAuthToken()", AUTH_TOKEN).what()));
...
{
  LOG_ERROR(fmt::runtime(Require("findWithAuthToken()", AUTH_TOKEN).what()));
...
{
  LOG_ERROR(fmt::runtime(Require("failedLoginAttempts()", THROTTLING).what()));
...
{
  LOG_ERROR(fmt::runtime(Require("setFailedLoginAttempts()", THROTTLING).what()));
}

WDateTime AbstractUserDatabase::lastLoginAttempt(const User& user) const
{
  LOG_ERROR(fmt::runtime(Require("lastLoginAttempt()", THROTTLING).what()));
...
void AbstractUserDatabase::setLastLoginAttempt(const User& user,
					       const WDateTime& t)
{
  LOG_ERROR(fmt::runtime(Require("setLastLoginAttempt()", THROTTLING).what()));
}
