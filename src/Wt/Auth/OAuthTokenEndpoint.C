...
void OAuthTokenEndpoint::handleRequest(const Http::Request &request, Http::Response &response)
{
  ...
  if (!authCode.checkValid() || authCode.redirectUri() != *redirectUri
    ...
    LOG_INFO("{{\"error\": \"invalid_grant\"}}: id:{} code: {} authcode: {} redirectUri: {}{} timestamp: {}{}",
             ...
             *redirectUri,(authCode.redirectUri() != *redirectUri ? " - invalid" : " - valid"),
             authCode.expirationTime().toString(), (WDateTime::currentDateTime() > authCode.expirationTime() ? ", expired" : ", not expired")
    );
    return;
  }
...
awaitable<void> OAuthTokenEndpoint::handleRequest(http::request &request, http::response &response)
{
#ifdef WT_TARGET_JAVA
  try {
#endif // WT_TARGET_JAVA
  ...
  auto grantType = request.getParameter("grant_type");
  auto redirectUri = request.getParameter("redirect_uri");
  std::string code { request.getParameter("code") };
  std::string clientId;
  std::string clientSecret;
  ClientSecretMethod authMethod = HttpAuthorizationBasic;
  ...
  // (only allowed for post methods)
  if (clientId.empty() && clientSecret.empty()) {
    auto clientIdParam = request.getParameter("client_id");
    auto clientSecretParam = request.getParameter("client_secret");
  ...
  if (code.empty() || clientId.empty() || clientSecret.empty() || grantType.empty() || redirectUri.empty()) {
    ...
    LOG_INFO("{{\"error\": \"invalid_request\"}}: code: {} clientId: {} clientSecret: {} grantType: {} redirectUri: {}",
             ...
             clientId,
             (clientSecret.empty() ? "MISSING" : "NOT MISSING"),
             (!grantType.empty() ? grantType : "NULL"),
             (!redirectUri.empty() ? redirectUri : "NULL"));
    co_return;
  }
  OAuthClient client = db_->idpClientFindWithId(clientId);
  if (!client.checkValid() || !client.verifySecret(clientSecret) || client.authMethod() != authMethod)
  {
    response.status(401);
    if (!authHeader.empty()) {
      if (client.authMethod() == HttpAuthorizationBasic)
  ...
  const OAuthClient &authClient = authCode.authClient();
  const std::string scope = authCode.scope();
  db_->idpTokenAdd(accessTokenValue, expirationTime, "access_token", scope, authCode.redirectUri(), user, authClient);
  db_->idpTokenRemove(authCode);
  response.status(200);
  Json::Object root;
  root["access_token"] = Json::Value(accessTokenValue);
