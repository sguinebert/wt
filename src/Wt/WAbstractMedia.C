                                  ...
                                  WAbstractMedia::Source &source, bool isLast)
{
    ...
    if (isLast && alternative_) {
        ...
        element->setAttribute("onerror", "", true);
    }
}
    ...
    if (all || flagsChanged_) {
        if ((!all) || flags_.test(PlayerOption::Controls))
            element.setAttribute("controls",
                                 flags_.test(PlayerOption::Controls) ? "controls" : "", true);
        if ((!all) || flags_.test(PlayerOption::Autoplay))
            element.setAttribute("autoplay",
                                 flags_.test(PlayerOption::Autoplay) ? "autoplay" : "", true);
        if ((!all) || flags_.test(PlayerOption::Loop))
            element.setAttribute("loop",
                                 flags_.test(PlayerOption::Loop) ? "loop" : "", true);
    }
    if (all || preloadChanged_) {
        switch (preloadMode_) {
        case MediaPreloadMode::None:
            element.setAttribute("preload", "none", true);
            break;
        default:
        case MediaPreloadMode::Auto:
            element.setAttribute("preload", "auto", true);
            break;
        case MediaPreloadMode::Metadata:
            element.setAttribute("preload", "metadata", true);
            break;
        }
    }
...
void WAbstractMedia::loadJavaScript()
{
    if (javaScriptMember(" WAbstractMedia").empty()) {
        ...
        auto js = fmt::format("new " WT_CLASS ".WAbstractMedia({}, {});", app->javaScriptClass(), jsRef());

        setJavaScriptMember(" WAbstractMedia", js);
    }
}
...
DomElement WAbstractMedia::createDomElement(WApplication *app)
{
    ...
    DomElement result = isInLayout() ? DomElement::createNew(DomElementType::DIV) : createMediaDomElement();
    if(isInLayout()) {
    ...
    if (!mediaId_.empty()) {
        ...
        if (sourcesChanged_) {
            ...
            for (std::size_t i = 0; i < sourcesRendered_; ++i)
                media.callJavaScript<true>(WT_CLASS ".remove('{}s{}');", mediaId_, i);
            sourcesRendered_ = 0;
            for (std::size_t i = 0; i < sources_.size(); ++i) {
            ...
            media.callJavaScript("{}.load();", jsMediaRef());
        }
