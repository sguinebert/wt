...
{
    appRoot_ = path;
    if (appRoot_[appRoot_.length() - 1] != '/'
#ifdef WT_WIN32
        && appRoot_[appRoot_.length() - 1] != '\\'
#endif
        ) {
        appRoot_ += "/";
    }
...
std::string_view Configuration::originalIPHeader() const {
    READ_LOCK;
...
std::string_view Configuration::redirectMessage() const
{
    READ_LOCK;
    return redirectMsg_;
}
...
std::string_view Configuration::uaCompatible() const
{
    READ_LOCK;
...
std::string_view Configuration::appRoot() const
{
    READ_LOCK;
...
void Configuration::readApplicationSettings(xml_node<> *app)
{
   ...
   * would do such a thing ? */
    ...
    for (unsigned i = 0; i < headMatters.size(); ++i) {
        ...
        headMatter_.emplace_back(ss.str(), userAgent);
    }
...
{
    return fmt::format(FMT_COMPILE("{}/{}"), runDirectory_, sessionId);
}
