...
//using http_socket = asio::basic_stream_socket<asio::ip::tcp, asio::io_context::executor_type>; //optmize performance by avoiding any_io_executor
using http_socket = asio::ip::tcp::socket; //any_io_executor
#ifdef WT_WITH_SSL
...
template <typename _Ty>
using is_middleware_classic_without_next = std::is_convertible<std::decay_t<_Ty>, std::function<void(context&)>>;

template <typename _Ty>
inline constexpr bool is_middleware_classic_without_next_v = is_middleware_classic_without_next<_Ty>::value;
...
//template <typename _Ty>
//using is_classic_lambda = std::is_same<std::decay_t<_Ty>, std::function<void(std::string&&)>>;

//template <typename _Ty>
//inline constexpr bool is_classic_lambda_v = is_classic_lambda<_Ty>::value;
...
//    }
    std::string lower_str(str.length(), '\0');
