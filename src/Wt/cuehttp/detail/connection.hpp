...
class base_connection : public std::enable_shared_from_this<base_connection<_Socket, _Ty>>, safe_noncopyable {
    template <typename> friend class h2_mixin;
...
public:
    ...
    base_connection(std::function<awaitable<void>(context&)> handler, asio::io_context& io_service) noexcept
        ...
        context_{[this](auto&& a) { return coro_reply_chunk(std::forward<decltype(a)>(a)); },
                 [this](auto&& a) { return coro_reply_chunk_sg(std::forward<decltype(a)>(a)); },
                 false,
                 [this](auto&& a) { return spawn_coro_ws_send(std::forward<decltype(a)>(a)); }},
