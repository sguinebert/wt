...
void on_accept(variant& state, lsquic_conn* handle, uint32_t max_conn)
{
  assert(handle);
  assert(std::holds_alternative<accepting>(state));
  std::get_if<accepting>(&state)->op->defer(error_code{}); // success
  state.emplace<open>(*handle, max_conn);
}
                              ...
                              bool is_http)
{
  ...
  if (o.accepting_streams.empty()) {
    ...
    if (o.incoming_streams.full()) {
      fprintf(stderr, "[ERROR] boost::circular_buffer incoming_streams is full\n");
