...
//    object&
//    get_object(object& ifnull) noexcept
//    {
//        if(!is_object())
//            return ifnull;
//        return obj_;
...
//    }
    ...
    */
    ...
    std::string
    get_string(std::string if_isnull) noexcept
    {
        if(!is_string())
                return std::move(if_isnull);
        return str_.c_str();
    }
    std::string
    get_string(std::string if_isnull) const noexcept
    {
        if(!is_string())
                return std::move(if_isnull);
        return str_.c_str();
    ...
    */
    ...
    std::int64_t
    get_int64(std::int64_t ifnull) const noexcept
    {
        if(!is_int64())
            return ifnull;
        return sca_.u;
    ...
    */
    ...
    std::uint64_t
    get_uint64(std::uint64_t ifnull) const noexcept
    {
        if(!is_uint64())
            return ifnull;
        return sca_.u;
    ...
    */
    ...
    double
    get_double(double ifnull) const noexcept
    {
        if(!is_double())
            return ifnull;
        return sca_.d;
...
*/
template<>
struct tuple_size< Wt::Json::key_value_pair >
...
*/
template<>
struct tuple_element<0, Wt::Json::key_value_pair>
{
    using type = Wt::Json::string_view const;
};

/** Tuple-like access for the value type of key_value_pair
*/
template<>
struct tuple_element<1, Wt::Json::key_value_pair>
{
    using type = Wt::Json::Value&;
};

/** Tuple-like access for the value type of key_value_pair
*/
template<>
struct tuple_element<1, ::Wt::Json::key_value_pair const>
{
    using type = ::Wt::Json::Value const&;
};
...
namespace std {
template <>
struct hash< ::Wt::Json::Value > {
    BOOST_JSON_DECL
    std::size_t
    operator()(::Wt::Json::Value const& jv) const noexcept;
};
} // std
#endif
