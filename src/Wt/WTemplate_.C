...
bool WTemplate_::_tr(const std::vector<WString>& args,
		    std::ostream& result)
{
  if (args.size() >= 1) {
...
bool WTemplate_::_block(const std::vector<WString>& args,
                              std::ostream& result)
{
  if (args.size() < 1)
    return false;
...
bool WTemplate_::_while(const std::vector<WString>& args,
			      std::ostream& result)
{
  if (args.size() < 2)
    return false;
...
bool WTemplate_::_id(const std::vector<WString>& args,
		    std::ostream& result)
{
  if (args.size() == 1) {
...
bool WTemplate_::Functions::tr(WTemplate_ *t, const std::vector<WString>& args,
			      std::ostream& result)
{
  return t->_tr(args, result);
}

bool WTemplate_::Functions::block(WTemplate_ *t, const std::vector<WString>& args,
                              std::ostream& result)
{
  return t->_block(args, result);
}

bool WTemplate_::Functions::while_f(WTemplate_ *t, const std::vector<WString>& args,
			      std::ostream& result)
{
  return t->_while(args, result);
}

bool WTemplate_::Functions::id(WTemplate_ *t, const std::vector<WString>& args,
			      std::ostream& result)
{
...
WTemplate_::WTemplate_(const WString& text)
  : previouslyRendered_(nullptr),
    newlyRendered_(nullptr),
    encodeInternalPaths_(false),
    encodeTemplateText_(true),
...
void WTemplate_::addFunction(const std::string& name, const Function& function)
{
  functions_[name] = function;
}

void WTemplate_::setCondition(const std::string& name, bool value)
{
...
bool WTemplate_::conditionValue(const std::string& name) const
{
  return conditions_.find(name) != conditions_.end();
}

std::unique_ptr<WWidget> WTemplate_::removeWidget(WWidget *widget)
{
...
void WTemplate_::iterateChildren(const HandleWidgetMethod& method) const
{
...
awaitable<void> WTemplate_::iterateChildren2(AsyncHandleWidgetMethod &&method) const
{
...
void WTemplate_::bindWidget(const std::string& varName,
...
std::unique_ptr<WWidget> WTemplate_::removeWidget(const std::string& varName)
{
  std::unique_ptr<WWidget> result;

  if (auto i = widgets_.find(varName); i != widgets_.end()) {
...
void WTemplate_::setWidgetIdMode(TemplateWidgetIdMode mode)
{
  widgetIdMode_ = mode;
}

void WTemplate_::bindEmpty(const std::string& varName)
{
  bindWidget(varName, nullptr);
}

void WTemplate_::bindString(const std::string& varName, const WString& value, TextFormat textFormat)
{
...
void WTemplate_::bindInt(const std::string& varName, int value)
{
  bindString(varName, std::to_string(value), TextFormat::UnsafeXHTML);
}

bool WTemplate_::resolveFunction(const std::string& name,
				const std::vector<WString>& args,
				std::ostream& result)
{

  if (auto i = functions_.find(name); i != functions_.end()) {
...
void WTemplate_::resolveString(const std::string& varName,
                               const std::vector<WString>& args,
                               std::ostream& result)
{
    /*
   ...
   */
    ...
    if (auto i = strings_.find(varName); i != strings_.end())
...
void WTemplate_::applyArguments(WWidget *w, const std::vector<WString>& args)
{
...
void WTemplate_::handleUnresolvedVariable(const std::string& varName,
                                         const std::vector<WString>& args,
                                         std::ostream& result)
{
  result << "??" << varName << "??";
...
WWidget *WTemplate_::resolveWidget(const std::string& varName)
{
  if (auto j = widgets_.find(varName); j != widgets_.end())
    return j->second.get();
  else
    return nullptr;
}

std::vector<WWidget *> WTemplate_::widgets() const
{
...
std::string WTemplate_::varName(WWidget *w) const
{
  for (WidgetMap::const_iterator j = widgets_.begin();
       j != widgets_.end(); ++j)
    if (j->second.get() == w)
...
void WTemplate_::setTemplateText(const WString& text, TextFormat textFormat)
{
  text_ = text;
...
void WTemplate_::updateDom(DomElement& element, bool all)
{
  if (changed_ || all) {
...
void WTemplate_::unrenderWidget(WWidget *w, DomElement &el)
{
...
std::string WTemplate_::encode(const std::string& text) const
{
...
void WTemplate_::renderTemplate(std::ostream& result)
{
  renderTemplateText(result, templateText());
}

bool WTemplate_::renderTemplateText(std::ostream& result, const WString& templateText)
{
  errorText_ = "";

  std::string text;
  if (encodeTemplateText_) //à supprimer
    text = encode(templateText.toXhtmlUTF8());
  else
    text = templateText.toXhtmlUTF8();

  //auto fmtresult = fmt::vformat(text, fmt_args_);
...
std::size_t WTemplate_::parseArgs(const std::string& text,
				 std::size_t pos,
...
void WTemplate_::format(std::ostream& result, const std::string& s,
		       TextFormat textFormat)
{
  format(result, WString::fromUTF8(s), textFormat);
}

void WTemplate_::format(std::ostream& result, const WString& s,
		       TextFormat textFormat)
{
...
void WTemplate_::propagateRenderOk(bool deep)
{
  changed_ = false;
...
void WTemplate_::enableAjax()
{
  WInteractWidget::enableAjax();
}

DomElementType WTemplate_::domElementType() const
{
...
void WTemplate_::setInternalPathEncoding(bool enabled)
{
...
void WTemplate_::setEncodeTemplateText(bool on)
{
  encodeTemplateText_ = on;
}

void WTemplate_::refresh()
{
