...
public:
    ...
    virtual std::unique_ptr<WPaintDevice> createPaintDevice(bool paintUpdate) = 0;
    virtual std::unique_ptr<WPaintDevice> getPaintDevice(bool paintUpdate) = 0;
    virtual void createContents(DomElement *element, std::unique_ptr<WPaintDevice> device) = 0;
    virtual void updateContents(std::vector<DomElement>& result, std::unique_ptr<WPaintDevice> device) = 0;
...
DomElement WPaintedWidget::createDomElement(WApplication *app)
{
    ...
    if (painter_->renderType() == WWidgetPainter::RenderType::InlineVml && isInline()) {
                     ...
                     std::unique_ptr<WPaintDevice> device)
{
    ...
    if (widget_->repaintFlags_.test(PaintFlag::Update)) {
        ...
        result.push_back(std::move(painter));
    } else {
    ...
    ::createContents(DomElement *result, std::unique_ptr<WPaintDevice> device)
{
    ...
    WApplication *app = WApplication::instance();
    {
        ...
        el->callJavaScript("new " WT_CLASS ".WPaintedWidget({},{});", app->javaScriptClass(), widget_->jsRef());
    }
    ...
    if (hasJsObjects) {
        ...
        el->callJavaScript("new " WT_CLASS ".WJavaScriptObjectStorage({},{});{:a}", app->javaScriptClass(), widget_->jsRef(), widget_->jsObjects_);
...
{
    WCanvasPaintDevice *canvasDevice
        = static_cast<WCanvasPaintDevice *>(device.get());

    if (widget_->sizeChanged_) {
        DomElement canvas = DomElement::getForUpdate('c' + widget_->id(), DomElementType::CANVAS);
        canvas.setAttribute("width", widget_->renderWidth_);
        canvas.setAttribute("height", widget_->renderHeight_);
        result.push_back(std::move(canvas));
    ...
    if (hasJsObjects) {
        ...
        el.callJavaScript("{}", widget_->jsObjects_);
        if (widget_->areaImage_) {
            widget_->areaImage_->setTargetJS(widget_->objJsRef());
            updateAreasJs = widget_->areaImage_->updateAreasJS();
        }
    ...
    (DomElement *result, std::unique_ptr<WPaintDevice> device)
{
    ...
    img.setAttribute("class", "unselectable", true);
    img.setAttribute("unselectable", "on", true);
    img.setAttribute("onselectstart", "return false;", true);
    img.setAttribute("onmousedown", "return false;", true);
                     ...
                     std::unique_ptr<WPaintDevice> device)
{
    ...
    DomElement img = DomElement::getForUpdate('i' + widget_->id(), DomElementType::IMG);
