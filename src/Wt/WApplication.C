...
namespace skeletons {
static constexpr char Wt_xmldata[] = {
#embed "../xml/wt.xml"
    , 0 // Add null terminator
};
static constexpr std::string_view Wt_xml(Wt_xmldata, sizeof(Wt_xmldata) - 1);
...
void WApplication::setConnectionMonitor(const std::string& jsFunction) {
    doJavaScript("{}._p_.setConnectionMonitor({})", javaScriptClass_, jsFunction);
}
...
void WApplication::removeMetaHeader(MetaHeaderType type, const std::string& name)
{
    if (environment().javaScript())
...
void WApplication::enableInternalPaths()
{
    if (!internalPathsEnabled_) {
        ...
        //  auto js = fmt::format("{}._p_.enableInternalPaths({});", javaScriptClass(), WWebWidget::jsStringLiteral(renderedInternalPath_));
        doJavaScript("{}._p_.enableInternalPaths({});", javaScriptClass(), WWebWidget::jsStringLiteral(renderedInternalPath_));
...
void WApplication::declareJavaScriptFunction(const std::string& name, const std::string& function)
{
    doJavaScript<false>("{}.{}={};", javaScriptClass_, name, function);
}

void WApplication::streamAfterLoadJavaScript(WStringStream& out)
{
    out << std::string_view(afterLoadJavaScript_);
    afterLoadJavaScript_.clear();
}
void WApplication::streamAfterLoadJavaScript(fmt::memory_buffer &out)
{
    fmt::format_to(std::back_inserter(out), "{}", afterLoadJavaScript_);
    afterLoadJavaScript_.clear();
}

void WApplication::streamBeforeLoadJavaScript(fmt::memory_buffer &out, bool all)
{
    streamJavaScriptPreamble(out, all);

    std::string_view str(beforeLoadJavaScript_);

    if (!all) {
        if (newBeforeLoadJavaScript_)
            fmt::format_to(std::back_inserter(out), "{}",  str.substr(beforeLoadJavaScript_.size() - newBeforeLoadJavaScript_));
    } else {
        fmt::format_to(std::back_inserter(out), "{}", beforeLoadJavaScript_);
...
void WApplication::streamBeforeLoadJavaScript(WStringStream& out, bool all)
{
    ...
    if (!all) {
        if (newBeforeLoadJavaScript_)
            out << str.substr(beforeLoadJavaScript_.size()
...
awaitable<void> WApplication::processEvents()
{
    ...
    doJavaScript("setTimeout(\"{}._p_.update(null,'none',null,true);\",0);", javaScriptClass_);
...
void WApplication::loadJavaScript(const char *jsFile, const WJavaScriptPreamble& preamble)
{
...
void WApplication::streamJavaScriptPreamble(fmt::memory_buffer &out, bool all)
{
    if (all)
        newJavaScriptPreamble_ = javaScriptPreamble_.size();
    ...
    for (unsigned i = javaScriptPreamble_.size() - newJavaScriptPreamble_;
        ...
        if (preamble.type == JavaScriptFunction) {
            fmt::format_to(std::back_inserter(out), "{}.{} = function() {{ return ({}).apply({}, arguments) }};\n",
                           scope, preamble.name, preamble.src, scope);
            // out << scope << '.' << (char *)preamble.name
            //     << " = function() { return ("
            //     << (char *)preamble.src << ").apply(" << scope << ", arguments) };\n";
        } else {
            fmt::format_to(std::back_inserter(out), "{}.{} = {};\n",
                           scope, preamble.name, preamble.src);
            // out << scope << '.' << (char *)preamble.name
            //     << " = " << (char *)preamble.src << ";\n";
        }
    }
