...
public:
    ...
    template <ASIO_NS::completion_token_for<void(async_mutex_lock)> LockToken>
    [[nodiscard]] auto async_scoped_shared_lock(LockToken &&token) {
        return ASIO_NS::async_initiate<LockToken, void(async_mutex_lock)>(detail::initiate_scoped_async_lock(this), token);
    }
    ...
    //https://grok.com/chat/0eead15b-f088-47dd-9852-d212b5af3493
    // void unlock_exclusive() {
    //     assert(m_state.load(std::memory_order_relaxed) == 0x2 || (m_state.load(std::memory_order_relaxed) & 0x3) == 0);
    //     auto old_state = m_state.exchange(0x1, std::memory_order_release);
    //     if ((old_state & 0x3) == 0) {
    //         auto *waiters_head = reinterpret_cast<detail::locked_waiter*>(old_state & ~0x3);
    //         while (waiters_head) {
    //             auto *next = waiters_head->next;
    //             waiters_head->next = m_waiters;
    //             m_waiters = waiters_head;
    //             waiters_head = next;
    //         }
    //         if (m_waiters) {
    //             auto *to_resume = m_waiters;
    //             m_waiters = to_resume->next;
    ...
    //     assert((old_state & 0x3) == 1 && (old_state >> 2) > 0);
    //     while (true) {
    //         auto new_state = old_state - 4;
    //         if (m_state.compare_exchange_weak(old_state, new_state, std::memory_order_release,
    //                                           std::memory_order_relaxed)) {
    //             if ((new_state >> 2) == 0 && m_waiters == nullptr) {
    //                 old_state = m_state.exchange(0x1, std::memory_order_acquire);
    //                 if ((old_state & 0x3) == 0) {
    //                     auto *waiters_head = reinterpret_cast<detail::locked_waiter*>(old_state & ~0x3);
    //                     while (waiters_head) {
    //                         auto *next = waiters_head->next;
    ...
    //                         waiters_head = next;
    //                     }
    //                     if (m_waiters /*&& m_waiters->type == detail::locked_waiter::LockType::Exclusive*/) {
    //                         auto *to_resume = m_waiters;
    //                         m_waiters = to_resume->next;
    //                         to_resume->completion();
    //                         delete to_resume;
