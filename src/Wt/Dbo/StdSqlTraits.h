...
struct WTDBO_API sql_value_traits<Enum, typename std::enable_if<std::is_enum<Enum>::value>::type>
...
struct sql_value_traits<std::optional<T>, void>
{
  ...
  static bool read(std::optional<T>& v, SqlStatement *statement, int column, int size) {
    T result;
    if (sql_value_traits<T>::read(result, statement, column, size)) {
      v = result;
      return true;
