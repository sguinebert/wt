...
template <class C>
Query<C> collection<C>::find() const
{
  if (type_ != RelationCollection)
    return Query<C>();
...
awaitable<int> collection<C>::count(C c) const
{
  ...
  Query<C> q = find().where(mapping->idCondition);
  c.obj()->bindId(q.parameters_);
  auto listResult = co_await q.resultList();
  std::size_t result = listResult ? listResult->size() : 0;
