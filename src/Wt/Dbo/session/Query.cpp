...
void AbstractQuery::bindParameters(Session *session, SqlStatement *statement) const
{
  ...
  for (auto& binder : parameters_)
    binder(statement, column);

  switch (session->limitQueryMethod_) {
  case LimitQuery::Limit:
    if (limit_ != -1)
      sql_value_traits<int>::bind(limit_, statement, column++, -1);
    if (offset_ != -1)
      sql_value_traits<int>::bind(offset_, statement, column++, -1);
    break;

  case LimitQuery::RowsFromTo:
    if (limit_ != -1 || offset_ != -1) {
      int from = offset_ == -1 ? 1 : offset_ + 1;
      sql_value_traits<int>::bind(from, statement, column++, -1);
      int to = (limit_ == -1) ? (1 << 30) : (from + limit_ - 1);
      sql_value_traits<int>::bind(to, statement, column++, -1);
    }
    break;
