...
public:
            ...
            SqlDateTimeType type) override
  {
    ...
    case DateTimeStorage::PseudoISO8601AsText: {
      ...
      if (type == SqlDateTimeType::Date){
        ...
        v = str;
        v.append(fmt::format(".{:03}", ms.count() % 1000));
             ...
             SqlDateTimeType type) override
  {
    ...
    LOG_ERROR("Sqlite3: unsupported datetime conversion mode in getResult()");
    return false;
  }
    ...
    db_(nullptr)
{
  ...
  if (err != SQLITE_OK) {
    LOG_ERROR("Sqlite3: open failed: {}", sqlite3_errmsg(db_));
    if (db_) {
      sqlite3_close(db_);
      db_ = nullptr;
    }
...
Sqlite3::Sqlite3(const Sqlite3& other)
  : Wt::Dbo::SqlConnectionBase(other),
    conn_(other.conn_),
    db_(nullptr)
{
  dateTimeStorage_[static_cast<unsigned>(SqlDateTimeType::Date)]
  ...
  if (err != SQLITE_OK) {
    LOG_ERROR("Sqlite3: copy/open failed: {}", sqlite3_errmsg(db_));
    if (db_) {
      sqlite3_close(db_);
      db_ = nullptr;
    }
...
const char *Sqlite3::dateTimeType(SqlDateTimeType type) const
{
  ...
  LOG_ERROR("Sqlite3: unsupported datetime storage mode, fallback to text");
  return "text";
}
...
SqlStatement *Sqlite3::getStatement(const std::string &id)
{
...
awaitable<dbo_result<void>> Sqlite3::startTransaction()
{
  co_return co_await executeSql("begin transaction");
}

awaitable<dbo_result<void>> Sqlite3::commitTransaction()
{
  co_return co_await executeSql("commit transaction");
}

awaitable<dbo_result<void>> Sqlite3::rollbackTransaction()
{
  co_return co_await executeSql("rollback transaction");
}
}
}
}
