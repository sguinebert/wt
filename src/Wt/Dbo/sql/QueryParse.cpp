...
#ifdef NO_SPIRIT_QUERY_PARSE
dbo_result<void> parseSql(const std::string& sql, SelectFieldLists& fieldLists)
{
  fieldLists.clear();
  ...
  if (selectPos != 0)
    return std::unexpected(dbo_error{
      DboErrc::Sql,
      "Session::query(): query should start with 'select '"
      " (sql='" + sql + "')",
      {},
      {},
      0,
...
dbo_result<void> parseSql(const std::string& sql, SelectFieldLists& fieldLists)
{
  ...
  if (success) {
    if (iter != end)
      return std::unexpected(dbo_error{
        DboErrc::Sql,
        "Error parsing SQL query: Expected end here: \""
        + std::string(iter, end) + "\"",
        {},
        {},
        0,
...
dbo_result<void> parseSql(const std::string &sql,
              SelectFieldLists &fieldLists)
{
  ...
  std::ostream errOut(&errBuf);

  using error_handler_type = x3::error_handler<std::string::const_iterator>;
  error_handler_type error_handler(iter, end, errOut);
