...
//        impl_->freeList.push_back(conn->clone());
      ...
      for (unsigned i = 1; i < size; ++i) {
          auto cloneResult = conn->clone(engine.get());
          if (!cloneResult) {
              std::cerr << "FixedSqlConnectionPool: clone failed: "
                        << to_string(cloneResult.error()) << '\n';
              break;
          }
          impl_->freeList.push_back(std::move(*cloneResult));
    ...
    std::unique_ptr<SqlConnection> FixedSqlConnectionPool::getConnection()
    {
      ...
      if (impl_->freeList.empty())
        return std::unexpected(dbo_error{
          DboErrc::Connection,
          "FixedSqlConnectionPool::getConnection(): "
...
//          timer.async_wait(asio::bind_cancellation_slot(cancel_signal_.slot(), use_nothrow_awaitable));
    ...
    {
      // Timeout is logged but no longer throws — callers check dbo_result
      std::cerr << "FixedSqlConnectionPool::getConnection(): timeout" << '\n';
    }
