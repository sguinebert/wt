...
MetaDbo<C> *Session::loadWithNaturalId(SqlStatement *statement, int& column)
{
  ...
  else if(i->second->obj_ == 0 && dbo->obj()){
     i->second->setObj(dbo->obj());
     i->second->setVersion(dbo->version());
     implUpdate<MutC>(*i->second);
     dbo->setSession(nullptr);
     dbo->setObj(nullptr);
     delete dbob;
     return i->second;
...
template <class C>
void Session::implUpdate(MetaDbo<C>& dbo)
{

  UpdateBaseAction<C> action(dbo, *getMapping<C>());
