...
class WT_API WDropZone : public WContainerWidget
{
...
public:
   ...
   */
    ...
    /*! \brief .
   *
   * \sa set max file size authorized
   */
    void setMaxSizeFile(unsigned size);
    /*! \brief
   *
   * \sa set max file number possible to drop
   */
    void setMaxFiles(unsigned size);
   ...
   */
    Signal<awaitable<void>(File*, std::string)>& uploadFailed() { return uploadFailed_; }
...
private:
    ...
    JSignal<std::string, std::string> errorUpload_;
    JSignal<std::string> uploadFinished_;
    ...
    Signal<awaitable<void>(File*, std::string)> uploadFailed_;
    ...
    friend class WDropZoneResource;
};
