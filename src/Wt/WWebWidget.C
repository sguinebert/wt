...
Signal<void()>& WWebWidget::childrenChanged()
{
    if (!otherImpl_)
        otherImpl_.reset(new OtherImpl(this));
...
void WWebWidget::setAttributeValue(const std::string& name, const WT_USTRING& value)
{
    if (!otherImpl_)
        otherImpl_.reset(new OtherImpl(this));
    ...
    auto i = otherImpl_->attributes_->find(name);
...
WT_USTRING WWebWidget::attributeValue(const std::string& name) const
{
    if (otherImpl_ && otherImpl_->attributes_) {
        if (auto i = otherImpl_->attributes_->find(name); i != otherImpl_->attributes_->end())
            return i->second;
    }
...
void WWebWidget::setJavaScriptMember(const std::string& name, const std::string& value)
{
    if (!otherImpl_)
        otherImpl_.reset(new OtherImpl(this));
...
{
    addJavaScriptStatement(JavaScriptStatementType::CallMethod, name + "(" + args + ");");
...
void WWebWidget::updateDom(DomElement& element, bool all)
{
   ...
   */
    ...
    if (flags_.test(BIT_GEOMETRY_CHANGED) ||
        ...
        (!flags_.test(BIT_HIDE_WITH_VISIBILITY) && flags_.test(BIT_HIDDEN_CHANGED)) ||
        all)
    {
        if (flags_.test(BIT_HIDE_WITH_VISIBILITY) || !flags_.test(BIT_HIDDEN)) {
            static constexpr std::string_view Inline = "inline";
            static constexpr std::string_view InlineTable = "inline-table";
            static constexpr std::string_view InlineBlock = "inline-block";
            static constexpr std::string_view Block = "block";
            static constexpr std::string_view Flex = "flex";
            static constexpr std::string_view InlineFlex = "inline-flex";
            ...
            else if (flags_.test(BIT_FLEX_BOX_CHANGED) && !display.empty())
                display = Empty;
           ...
           */
            if (layoutImpl_->zIndex_ > 0) {
                ...
                if (all && app->environment().agent() == UserAgent::IE6
                    ...
                    i.setAttribute("title", "Popup Shim", true);
                    i.setAttribute("tabindex", "-1", true);
                    i.setAttribute("frameborder", "0", true);

                    fmt::format_to(app->autoJavaScript(), FMT_COMPILE("{{var w={};if(w&&!{}.isHidden(w)){{var i={}.getElement('{}');"
                                                          "i.style.width=w.clientWidth+'px';i.style.height=w.clientHeight+'px';}}}}"),
                                   jsRef(), WT_CLASS, WT_CLASS, i.id());
   ...
   */
    ...
    if (!all && transientImpl_) {
        for (unsigned i = 0; i < transientImpl_->addedStyleClasses_.size(); ++i)
            element.callJavaScript(WT_CLASS ".$('{}').classList.add('{}');", id(), transientImpl_->addedStyleClasses_[i].toUTF8());

        for (unsigned i = 0; i < transientImpl_->removedStyleClasses_.size(); ++i)
            element.callJavaScript(WT_CLASS ".$('{}').classList.remove('{}');", id(), transientImpl_->removedStyleClasses_[i].toUTF8());
        ...
        if (!transientImpl_->childRemoveChanges_.empty()) {
            if ((int)children().size() != transientImpl_->addedChildren_
                ...
                for (unsigned i = 0; i < transientImpl_->childRemoveChanges_.size();
                    ...
                    if (js[0] == '_')
                        element.callJavaScript<true>(WT_CLASS ".remove('{}');", js.substr(1));
                    else
    ...
    if (all || flags_.test(BIT_SELECTABLE_CHANGED)) {
        if (flags_.test(BIT_SET_UNSELECTABLE)) {
            ...
            element.setAttribute("unselectable", "on", true);
            element.setAttribute("onselectstart", "return false;", true);
        } else if (flags_.test(BIT_SET_SELECTABLE)) {
            element.addPropertyWord(Property::Class, "selectable");
            element.setAttribute("unselectable", "off", true);
            element.setAttribute("onselectstart",
                                 "event.cancelBubble=true; return true;", true);
        }
    ...
    if (otherImpl_) {
        if (otherImpl_->attributes_) {
            if (all) {
                for (auto &[key, value] : *otherImpl_->attributes_)
                    if (key == "style")
                        element.setProperty(Property::Style, value.toUTF8());
                    else
                        element.setAttribute(key, value.toUTF8());
            } else if (transientImpl_) {
                for (auto& attr : transientImpl_->attributesSet_) {
                    if (attr == "style")
    ...
    if (flags_.test(BIT_HIDE_WITH_VISIBILITY)) {
        if (flags_.test(BIT_HIDDEN_CHANGED)
            ...
            if (flags_.test(BIT_HIDDEN)) {
                element.callJavaScript(WT_CLASS ".$('{}').classList.add('Wt-hidden');", id());
                ...
                element.callJavaScript(WT_CLASS ".$('{}').classList.remove('Wt-hidden');", id());
	 ...
	 */
                ...
                //    << "');";
                element.callJavaScript(WT_CLASS ".animateDisplay({},'{}',{},{},{},'{}');",
                                       app->javaScriptClass(),
                                       id(),
                                       transientImpl_->animation_.effects().value(),
                                       (int)transientImpl_->animation_.timingFunction(),
	 ...
	 */
                ...
                //    << "');";
                element.callJavaScript(WT_CLASS ".animateVisible('{}',{},{},{},'{}','{}','{}','{}');",
                                       id(),
                                       transientImpl_->animation_.effects().value(),
                                       (int)transientImpl_->animation_.timingFunction(),
                                       transientImpl_->animation_.duration(),
    ...
    if (flags_.test(BIT_GOT_FOCUS)) {
        ...
        element.callJavaScript("setTimeout(function() {{"
                               """var o = {};if (o) {{if (!o.classList.contains('{}')) {{try {{o.focus();}} catch (e) {{}} }} }} }}, {});",
                               jsRef(),
    ...
    if (all || flags_.test(BIT_SCROLL_VISIBILITY_CHANGED)) {
        ...
        if (!app->javaScriptLoaded(SCROLL_JS) && scrollVisibilityEnabled()) {
            ...
            element.callJavaScript("if (!" WT_CLASS ".scrollVisibility){{"
                                   WT_CLASS ".scrollVisibility = new " WT_CLASS ".ScrollVisibility({});}}",
                                   app->javaScriptClass());
        }
        ...
        if (scrollVisibilityEnabled()) {
            ...
            // ss << "});";
            element.callJavaScript(WT_CLASS ".scrollVisibility.add({{el:{},margin:{},visible:{}}});",
                                   jsRef(),
    ...
    if (name[0] != ' ') {
        if (name == WT_RESIZE_JS && otherImpl_->resized_) {
            ...
            if (value.length() > 1) {
                ...
                element.callMethod("{}={}_p_.propagateSize", name, WApplication::instance()->javaScriptClass());
            }
...
void WWebWidget::getDomChanges(std::vector<DomElement>& result, WApplication *app)
{
    ...
    //DomElement e(DomElement::Mode::Update, domElementType(), id());
    updateDom(e, false);
     ...
     */
        if (app->session()->renderer().preLearning()) {
            ...
            if (!app->session()->renderer().visibleOnly()) {
                ...
                //DomElement stub(DomElement::Mode::Update, DomElementType::SPAN, id());
                DomElement& stub = result.emplace_back(DomElement::Mode::Update, DomElementType::SPAN, id());
                ...
                stub.unstubWith(std::move(realElement), !flags_.test(BIT_HIDE_WITH_OFFSETS));
...
{
    if (!app->environment().agentIsSpiderBot() || id_) {
        if (!flags_.test(BIT_FORM_OBJECT))
            element->setId(id());
        else
            element->setName(id());
...
{
    setRendered(true);
    auto type = (otherImpl_ && otherImpl_->elementTagName_) ? DomElementType::OTHER : domElementType();
    DomElement result(DomElement::Mode::Create, type);

    if (otherImpl_ && otherImpl_->elementTagName_) {
        //result.type_ = DomElementType::OTHER;
...
DomElement WWebWidget::createStubElement(WApplication *app)
{
   ...
   */
    ...
    DomElement stub(DomElement::Mode::Create, DomElementType::SPAN, id());
    if (!flags_.test(BIT_HIDE_WITH_OFFSETS)) {
