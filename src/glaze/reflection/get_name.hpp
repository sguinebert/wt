...
namespace glz::detail
{
   ...
   {
      return std::string_view{GLZ_PRETTY_FUNCTION};
   }
...
#pragma clang diagnostic ignored "-Weverything"
   ...
   inline constexpr std::string_view get_name_impl = std::string_view{};
#pragma clang diagnostic pop
   ...
   struct GLAZE_REFLECTOR
   {
      int GLAZE_FIELD;
          ...
          // Safe helpers for parsing pretty-function text
   consteval std::string_view safe_field_end(std::string_view name) {
      const auto pos = name.find("GLAZE_FIELD");
      if (pos == std::string_view::npos) return {};
      const auto start = pos + sizeof("GLAZE_FIELD") - 1;
      if (start > name.size()) return {};
      return name.substr(start);
   }
   consteval char safe_field_begin(std::string_view name) {
      const auto pos = name.find("GLAZE_FIELD");
      if (pos == std::string_view::npos || pos == 0) return ':'; // benign default
      return name[pos - 1];
   }
   struct reflect_field
   {
      static constexpr std::string_view name = get_name_impl<0, GLAZE_REFLECTOR>;
      static constexpr std::string_view end  = safe_field_end(name);
      static constexpr char            begin = safe_field_begin(name);
