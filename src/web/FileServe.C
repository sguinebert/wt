            ...
            if (std::strncmp(s, "_$_", 3) == 0) {
                if (currentVar[0] == '$') {
                    ...
                    if (fname == "endif") {
                        ...
                        auto i = conditions_.find(farg);

                        if (i == conditions_.end())
                            throw WException("Internal error: could not find condition: " + farg);
                        bool c = i->second;
                    ...
                    auto i = vars_.find(currentVar);

                    if (i == vars_.end())
                        throw WException("Internal error: could not find variable: " + currentVar);
