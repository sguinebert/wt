...
inline bool regex_search(std::string_view sv,
  ...
  bool regexMatch(std::string_view text, const std::regex &e)
  {
      return std::regex_search(text.begin(), text.end(), e);
  }
#else
...
namespace Wt
{
  ...
  void CgiParser::parse(Wt::http::context *context, ReadOption readOption)
  {
    ...
    {
      if (len > maxFormData_)
        throw WException(fmt::format("Oversized application/x-www-form-urlencoded ({})", len));
      ...
      // if(request.isWebSocketMessage())
      //   std::cerr << "formQueryString (len=" << len << "): " << formQueryString << std::endl;
      ...
      //deprecated handled request.parseFormUrlEncoded
    ...
    if (readOption != ReadHeadersOnly && type.find("multipart/form-data") != std::string_view::npos)
    {
        if (meth != "POST")
        {
            throw WException("Invalid method for multipart/form-data: " + std::string(meth));
  ...
  void CgiParser::parse(std::string_view message, std::string_view sessionID, http::context *context, ReadOption option)
  {
    ...
    parameters.append("wtd", std::string(sessionID));
    parameters.append(std::string("request"), "jsupdate");
...
//    std::string_view meth = "POST";
         ...
         (queryString.find("&contentType=x-www-form-urlencoded") != std::string_view::npos))*/)
    {
      ...
      if (!message.empty())
      {
        request.parseFormUrlEncoded(message); //append to ada::url_search_params the x-www-form-urlencoded body string
      }
    }
  ...
  bool CgiParser::parseBody(Wt::http::context *context, std::string_view boundary)
  {
    std::string value;

    readUntilBoundary(boundary, 2,
    ...
    else
    {
      if (!currentKey_.empty())
      {
        ...
        context->req().query().append(std::move(currentKey_), std::move(value));
      }
    }

    currentKey_.clear();

    if (bodypart_.substr(boundary.length(), 2) == "--")
        return false;
    // if (std::string_view(buf_ + boundary.length(), 2) == "--")
    //   return false;
  ...
  bool CgiParser::parseHead(Wt::http::context *context)
  {
    ...
    for (unsigned current = 0; current < head.length();)
    {
      /* read line by line */
      auto i = head.find("\r\n", current);
      auto text = head.substr(current, (i == std::string_view::npos
  ...
  */
  void CgiParser::readUntilBoundary(std::string_view boundary,
                                    int tossAtBoundary,
                                    std::string *resultString,
                                    std::ostream *resultFile)
  {

      if(auto bpos = bodypart_.find(boundary); bpos != std::string_view::npos) {

          // size_t end_pos;
          // if (tossAtBoundary >= 0) {
          //     end_pos = bpos > static_cast<size_t>(tossAtBoundary) ? bpos - tossAtBoundary : 0;
          // } else {
          //     end_pos = std::min(bpos + static_cast<size_t>(-tossAtBoundary), bodypart_.size());
          // }

          auto extract = bodypart_.substr(0, bodypart_.size() - bpos - tossAtBoundary);
          if (resultString)
              resultString->append(extract);
          if (resultFile)
              resultFile->write(extract.data(), extract.size());
          bodypart_.remove_prefix(extract.size());
          return;
      }
      ...
      // if (resultString)
      //     (*resultString).append(buf_, bpos - tossAtBoundary);
      // if (resultFile)
  ...
  std::string_view CgiParser::readUntilBoundary(std::string_view boundary, int tossAtBoundary)
  {
  ...
  int CgiParser::index(std::string_view search)
  {
    ...
    if (auto i = bufS.find(search); i != std::string_view::npos)
        return i;
    return -1;
  }
