...
namespace Wt {
  namespace FileUtils {
    ...
    extern WT_API
    std::vector<unsigned char> fileHeader(const std::string &fileName, unsigned size);
    ...
    extern void listFiles(const std::string &directory, std::vector<std::string> &files);
    ...
    extern void appendFile(const std::string &srcFile, const std::string &targetFile);
  }
}
