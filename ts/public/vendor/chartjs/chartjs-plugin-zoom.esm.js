 ...
 */
/**
 * Native JavaScript replacements for Chart.js helpers
 * No external dependencies required
 */

/**
 * Iterate over array or object
 * @param {Array|Object} loopable - The object or array to iterate over
 * @param {Function} fn - Function to call for each item
 * @param {Object} [thisArg] - Context to set `this` to in the callback
 */
export function each(loopable, fn, thisArg = undefined) {
  if (loopable) {
    if (Array.isArray(loopable)) {
      ...
      Object.keys(loopable).forEach(key => {
        fn.call(thisArg, loopable[key], key);
      });
    }
  }
}
...
/**
 * Return the first defined value, or the second value if the first is undefined
 * @param {*} value - The primary value
 * @param {*} defaultValue - The fallback value
 * @returns {*} The first value if defined, otherwise the second value
 */
...
/**
 * Determine if two values are approximately equal, within a tolerance
 * @param {Number} a - First value
 * @param {Number} b - Second value
 * @param {Number} epsilon - Tolerance (defaults to 1e-6)
 * @returns {Boolean} True if values are approximately equal
 */
export function almostEquals(a, b, epsilon = 1e-6) {
  return Math.abs(a - b) < epsilon;
}

/**
 * Safely call a function with given arguments if it exists
 * @param {Function} fn - The function to call
 * @param {Array} args - Arguments to pass to the function
 * @returns {*} Return value from the function or undefined
 */
...
export function sign(x) {
  return Math.sign ? Math.sign(x) : (x === 0 ? 0 : x > 0 ? 1 : -1);
}

/**
 * Get the position of an event relative to a canvas
 * @param {Event} event - The mouse or touch event
 * @param {HTMLCanvasElement|Object} canvas - The canvas or chart object
 * @returns {Object} The x and y coordinates
 */
export function getRelativePosition(event, chart) {
  ...
  // Scale coordinates if CSS size and canvas render size don't match
 ...
 * @param {Object} area - The area to check against {left, right, top, bottom}
 * @returns {Boolean} True if point is inside area
 */
...
// Modern replacement for Hammer.js functionality in chartjs-plugin-zoom
class TouchGestureManager {
  ...
  trigger(eventName, event) {
    ...
    this.handlers[eventName].forEach(callback => callback(event));
  }
  ...
  handlePointerDown(e) {
    ...
    // Add move and up handlers to document (capture events outside canvas)
    ...
    if (this.pointers.size === 1) {
      ...
      this.lastCenter = { ...this.startCenter };
      
      // Emit center as event.center for compatibility
      this.trigger('panstart', {
        center: this.startCenter,
        pointers: Array.from(this.pointers.values()),
        target: this.element,          // Add reference to the canvas element
        originalEvent: e               // Store original event for reference  
      });
  ...
  handlePointerUp(e) {
    ...
    if (this.pinching && this.pointers.size < 2) {
      ...
      this.trigger('pinchend', {
        scale: this.currentDistance / this.startDistance
      });
    }
    
    // If no more pointers, clean up
...
// Replace Hammer with our implementation in the plugin
const gestureManagers = new WeakMap();

// Replacement for startGestureTracking function
function startGestureTracking(chart, options) {
  const state = getState(chart);
  const canvas = chart.canvas;
  ...
  if (zoomOptions && zoomOptions.pinch.enabled) {
    manager.on('pinchstart', (e) => startPinch(chart, state, e));
    manager.on('pinch', (e) => handlePinch(chart, state, e));
    manager.on('pinchend', (e) => endPinch(chart, state, e));
  }
  
  if (panOptions && panOptions.enabled) {
    const enableCheck = createEnabler(chart, state);
    
    manager.on('panstart', (e) => {
      if (enableCheck(null, {srcEvent: e.originalEvent})) {
        startPan(chart, state, e);
      }
    });
    
    manager.on('panmove', (e) => handlePan(chart, state, e));
    manager.on('panend', () => endPan(chart, state));
  }
  
  gestureManagers.set(chart, manager);
}

// Replacement for stopGestureTracking function
function stopGestureTracking(chart) {
  const manager = gestureManagers.get(chart);
  if (manager) {
    manager.destroy();
    gestureManagers.delete(chart);
  }
}
...
var plugin = {
  ...
  start: function(chart, _args, options) {
    ...
    if (Object.prototype.hasOwnProperty.call(options.zoom, 'overScaleMode')
      ...
      startGestureTracking(chart, options);
